  ...
  <div class="bg-green-50 dark:bg-gray-800 h-full w-full">
    <div class="flex">
      <div class="px-6 justify-center">
        <div>
          <div class="flex justify-evenly items-center my-6 gap-5 flex-wrap w-full">
        ...
        <div v-if="isData == false">
          <div colspan="11" class="col-span-full">
            <div class="text-gray-800 dark:text-white block py-11 px-11">
              ...
              >
                No thing to show. once you posts new events, posted events will be
                displayed.
              </p>
            </div>
          </div>
        ...
        <div
          v-if="isData == true"
          class="pl-5"
          v-for="event in news.slice().reverse()"
          :key="event.id"
        >
          <div
            v-if="
              event.kebele == kebele_address &&
              event.newsSource == 'User' &&
              event.poster_email == user_email
            "
          >
...
const getNews = async () => {
  try {
    ...
    for (let x in news.value) {
      if (
        news.value[x].kebele == kebele_address &&
        news.value[x].newsSource == "User" &&
        news.value[x].poster_email == user_email
      ) {
        isData.value = true;
      }
    }

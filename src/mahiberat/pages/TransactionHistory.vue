          ...
          <form action="">
            <div class="flex gap-0">
              ...
              <input
                type="submit"
                value="Search"
                @click="filterTransactionHistory"
        ...
        <table class="table-auto w-full text-gray-700">
          ...
          <tbody>
            <tr v-for="item in datas" :key="item.order_id" class="hover:bg-slate-200">
              <td v-if="item.kebele == kebele && item.payStatus == 1" class="pl-2">
                {{ item.fName }}
              </td>
              <td v-if="item.kebele == kebele && item.payStatus == 1" class="pl-2">
                {{ item.faName }}
...
const filterTransactionHistory = async () => {
  localStorage.setItem("filter_transaction_history", filterTransaction.value);
  router.push(`/mahiberat/transactionHistory/${filterTransaction.value}`);
};

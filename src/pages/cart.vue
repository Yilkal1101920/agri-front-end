...
const getOrderedProductById = async (id) => {
  try {
    ...
    orderDifference.value = orderArray.value[getOrder.data.order_id];
    ...
    await updateTransactionByEmailAndProductNameConfirm(productId.value, cart_email);
...
const getProductbyproductidConfirm = async (id) => {
  try {
    const product = await axios.get(`http://localhost:5000/products/${id}`);
    productName.value = product.data.product_id;

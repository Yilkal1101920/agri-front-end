...
const login=()=>{
signInWithEmailAndPassword(getAuth(),email.value,password.value).then((res)=>{
  alert("success");
  alert(res.user.email);
  localStorage.setItem('user_id', 1);
  // localStorage.setItem('user_log', 1);
  localStorage.setItem('user_email', res.user.email);
  logoutSto.logStore = true; // show log out button
    ...
    <div class="flex justify-center items-center flex-wrap h-full g-6 text-gray-800">
      ...
      <div class="md:w-8/12 lg:w-5/12 lg:ml-20">
        <form>
          ...
          <button
            type="submit"
            class="inline-block px-7 py-3 bg-blue-600 text-white font-medium text-sm leading-snug uppercase rounded shadow-md hover:bg-blue-700 hover:shadow-lg focus:bg-blue-700 focus:shadow-lg focus:outline-none focus:ring-0 active:bg-blue-800 active:shadow-lg transition duration-0 ease-in-out w-full"
